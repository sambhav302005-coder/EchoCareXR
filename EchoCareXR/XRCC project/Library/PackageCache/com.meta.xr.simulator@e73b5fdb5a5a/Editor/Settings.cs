...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
...

            {
                ...
                var selectedIndex = installedVersions.IndexOf(PreferredVersion);
                if (selectedIndex == -1)
                {
                    PreferredVersion = Utils.GetMaxVersion(installedVersions);
                    selectedIndex = installedVersions.IndexOf(PreferredVersion);
                }

                EditorGUI.BeginChangeCheck();
                var index = EditorGUILayout.Popup("Selected Version", selectedIndex, installedVersions.ToArray());
                if (EditorGUI.EndChangeCheck())
                {
